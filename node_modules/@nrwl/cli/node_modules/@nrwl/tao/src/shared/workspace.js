...
class Workspaces {
    ...
    readWorkspaceConfiguration() {
        ...
        const workspace = workspacePath && (0, fs_1.existsSync)(workspacePath)
            ...
            : buildWorkspaceConfigurationFromGlobs(nxJson, globForProjectFiles(this.root), (path) => (0, fileutils_1.readJsonFile)((0, path_1.join)(this.root, path)));
...
function globForProjectFiles(root) {
    if (projectGlobCache)
    ...
    const globResults = (0, fast_glob_1.sync)('**/@(project.json|package.json)', {
...
function buildWorkspaceConfigurationFromGlobs(nxJson, projectFiles = globForProjectFiles(app_root_1.appRootPath), // making this parameter allows devkit to pick up newly created projects
    ...
    for (const file of projectFiles) {
        ...
        if (fileName === 'package.json') {
            ...
            if (!projects[name]) {
                ...
                logger_1.logger.error(`Skipping project found at ${directory} since project ${name} already exists! To have multiple Nx projects publish to the same npm package, you must use project.json based configuration`);
                throw new Error();
            }
            projects[name] = config;
        }
